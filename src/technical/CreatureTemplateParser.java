...
public class CreatureTemplateParser {
	public static CreatureTemplate parseFile(File f) {
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				if (w.matches("\\w+\\s\\d+/\\d+\\s\\d+\\s((AC)|(REF)|(WILL)|(FORT))\\s\\d+")) {
					parseRangedAttackTwo(attacks, w);
				}
			}
	...
	private static void parseRangedAttackTwo(List<Attack> attacks, String w) {
		String[] tt = w.split(" ");
		final String attackName = tt[0];
		...
		final int bonus = Integer.parseInt(tt[4]);
